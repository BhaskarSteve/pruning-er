...
OUT = "data/s1k-1.1_chatml.jsonl"
os.makedirs(os.path.dirname(OUT), exist_ok=True)

ds = load_dataset("simplescaling/s1K-1.1", split="train")  # fields: question, attempt, thinking_trajectories[0]  [s1K]
with open(OUT, "w", encoding="utf-8") as f:
    for ex in ds:
        q = ex["question"].strip()
        ans = str(ex["gemini_attempt"]).strip()
